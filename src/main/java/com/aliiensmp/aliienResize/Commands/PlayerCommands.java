...
public class PlayerCommands extends BaseCommand {
     ...
     */
    @Subcommand("|menu")
    @CommandPermission("aliien.resize.use")
    ...
    public void clearSize(Player target) {
        if (!plugin.getResizeUtils().hasEnoughSpace(target, 1.0)) {
            MessageUtils.send(target, Messages.PREFIX, Messages.RESIZE_FAIL);
            return;
        }
        ...
        target.getScheduler().run(plugin, scheduledTask -> {
            ...
            MessageUtils.send(target, Messages.PREFIX, Messages.RESIZE_DEFAULT);
        }, null);
    }
}